...
- **完整碗型建模** —— 内联 SVG 画的碗：碗口金边、内腔、碗底，与骰子共用同一个相机
  （相机仰角 72° = 90° − 骰子平面倾角 18°）。这个角度下**外壁和底座被碗口自己挡住**，
  画了也看不见 —— 所以碗只画到能被看见的部分为止。这是投影算出来的结论，不是省事
...
- `?selftest=exhaustive|flow|steal|input|rules|dice|solo|quota` —— 自检探针（8 组共 246 项断言）
