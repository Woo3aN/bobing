...
- **联机对战** —— 开局页点「🌐 联机对战」**创建房间**，把 4 位房间号发给朋友即可加入；
  ...
  **掉线 / 挂机全自动处置**（不用任何人手动操作）：
  ...
  | 超过 2 分钟（从**第一次被代博**算起） | 视为彻底断线：**只提示一次**「已移出本局」，之后静默跳过、**连本人也无法再取消**（真移出）；奖品按已博到的结算。重连后可以**观战**（牌局实时可见，掷骰按钮只读），下一局照常参与 |
  | 所有人都被移出 | 房间**自动解散**（定时器兜底检查：全员超时未回就没必要留着） |
  | **没有任何自由玩家**（最后一人也掉线/挂机） | **不卡在"代博中"**：服务端把全员标记为已进入跳过阶段 → 界面显示「全员掉线 · 本局已停止，2 分钟后自动解散」；2 分钟内有人回来即恢复 |
...
- **免费额度实测口径**（Workers Free，限额取自 Cloudflare 官方定价页；
  ...
  | 资源 | 免费额度/天 | 每局消耗 | 折合每天 |
  |---|---|---|---|
  | **SQLite 行写** | 100,000 | **≈ 270–285 行**（每掷一次存一次快照，put 单 key 计 1 行；托管/取消/点名等状态写入每局再多约 10 行） | **≈ 370 局** ← 瓶颈 |
