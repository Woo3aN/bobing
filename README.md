...
- **联机对战** —— 开局页点「🌐 联机对战」**创建房间**，把 4 位房间号发给朋友即可加入；
  ...
  **掉线 / 挂机全自动处置**（不用任何人手动操作）：
  ...
  | 超过 2 分钟（从**第一次被代博**算起） | 视为彻底断线：**只提示一次**「已移出本局」、
  之后静默跳过、**连本人也无法再取消**（真移出）、奖品按已博到的结算；重连后可以**观战**
  （能看到牌局实时推进，但掷骰按钮变成只读提示），下一局照常参与 |
  | 所有人都被移出 | 房间**自动解散**（定时器兜底检查：全员超时未回就没必要留着） |
  | **没有任何自由玩家**（最后一人也掉线/挂机） | **不卡在"代博中"**：服务端把全员标记为
  已进入跳过阶段 → 界面显示「全员掉线 · 本局已停止，2 分钟后自动解散」；2 分钟内有人回来即恢复 |
